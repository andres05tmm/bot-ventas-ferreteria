...
- **Confirmar gasto/fiado/abono (hecho):** las ventas ya tenían 2 pasos (método de pago); gasto/fiado/abono se registraban de una. Ahora `ai/tools.py::confirmacion_mutaciones_voz` retiene la mutación y devuelve pregunta hablada ("Voy a registrar un gasto de 5000 en refrigerio. ¿Confirmás?"); se registra recién el turno siguiente, cuando `es_afirmacion_voz` (en `ai/__init__.py`) detecta el "sí" y deja pasar la tool (Claude la reemite porque la app manda el historial del loop). **Sin tocar la app Kotlin** (el loop ya manda cada turno). Backend-only, scoped a voz. Turno mixto (venta+gasto) no se intercepta. 21 tests nuevos en `tests/test_ai_tools.py`. Falta probar en device (que Claude reemita la tool al confirmar).
...
- **Pendiente:** normalización de transcripción (`_normalizar_con_haiku`), afinar VAD.
